  ...
  tr: {
    ...
    projects: [
      ...
      },
      {
            ...
            "overview": "<strong>Proje Künyesi & TUSAŞ LIFT UP Programı:</strong><br>\n• <strong>Proje Başlığı:</strong> Muharip Hava Araçlarında Uçuş Performansı Veri Setinin Makine Öğrenmesi Yöntemleriyle Modellenmesi ve Kıyaslanması<br>\n• <strong>Sponsor & Program:</strong> TUSAŞ (Türk Havacılık ve Uzay Sanayii A.Ş.) 2025-2026 LIFT UP Sanayi Odaklı Lisans Bitirme Projeleri Programı<br>\n• <strong>Sanayi Danışmanı:</strong> Zeynel Abidin AYDOĞAN (TUSAŞ - Havacılık Mühendisliği)<br>\n• <strong>Akademik Danışman:</strong> Dr. Aziz KABA (Eskişehir Teknik Üniversitesi - Pilotaj Bölümü)<br>\n• <strong>Proje Ekibi:</strong> Ahmet Soner GÜLEÇ (Lider Öğrenci - ESTÜ Havacılık Elektrik Elektroniği), Umut ÇÖRDÜK (ESTÜ HEE), Ufuk ÖZKAN (ESTÜ Bilgisayar Mühendisliği), Sema ÜNAL (ESTÜ Endüstri Mühendisliği).<br><br>\n<strong>Mühendislik Problemi ve Amacı:</strong><br>\nMuharip hava araçlarında (örn: MMU KAAN, F-18) görev başarısı; farklı irtifa, hız, brüt ağırlık ve sürükleme koşullarında uçuş performans parametrelerinin (Özgül Menzil / Specific Range, Yakıt Akışı, Tırmanma Oranı vb.) milisaniye hassasiyetinde hesaplanmasına bağlıdır. Geleneksel Arama Tablosu (Lookup Table - LUT) yöntemleri deterministik ve açıklanabilirdir; ancak çok boyutlu değişken uzayında tablo çözünürlüğü arttıkça bellek gereksinimi katlanarak yükselir. Tablo aralığı büyütüldüğünde ise ara değer interpolasyon sapmaları oluşur.<br><br>\nBu projenin temel amacı; Uçak Uçuş El Kitaplarındaki (AFM) analog nomogram grafiklerinden U-Net evrişimli ağ ve OCR (Optik Karakter Tanıma) entegrasyonu ile üretilen master veri kümesi üzerinde, geleneksel Kübik İnterpolasyon referans yöntemi ile yapay zeka modellerini (XGBoost ve Tabular Transformer) doğruluk, çıkarım hızı, bellek ayak izi ve işlemci yükü açısından donanım-farkındalıklı olarak kıyaslamaktır.",
            "architecture": "<strong>1. Aşama: Veri Elde Etme ve Dijitalleştirme Pipeline'ı (U-Net & OCR):</strong><br>\n• <strong>U-Net Segmentasyonu:</strong> Orijinal AFM nomogram grafik görsellerindeki hedef performans eğrileri, U-Net evrişimli sinir ağı mimarisi ile piksel düzeyinde arka plan ızgara çizgilerinden ve harf bindirmelerinden ayrıştırılmıştır.<br>\n• <strong>OCR Entegrasyonu:</strong> Eksen sayısal değerleri, etiketler ve koordinat bilgileri Optik Karakter Tanıma ile okunmuş ve %2 sapma sınırında denormalize edilmiştir.<br>\n• <strong>Master Veri Seti (Tidy Data):</strong> 5 temel giriş değişkeni (Mach Sayısı, İrtifa, Brüt Ağırlık, Sürükleme İndeksi, Motor Konfigürasyonu) ve hedef çıktı (Özgül Menzil) için yapılandırılmış veri tablosu oluşturulmuştur.<br><br>\n<strong>2. Aşama: Matematiksel Modelleme & Yapay Zeka Mimarileri:</strong><br>\n• <strong>1. Kübik İnterpolasyon (Referans Taban):</strong> Ardışık noktalar arasında 3. derece kübik spline polinomları S_i(x) = a_i + b_i(x-x_i) + c_i(x-x_i)^2 + d_i(x-x_i)^3 kullanılarak birinci ve ikinci türev sürekliliği korunan yumuşak tablo ara değer üretimi yapılmıştır.<br>\n• <strong>2. XGBoost Regresyon Modeli:</strong> Karar ağaçlarının toplamsal bileşimi y_i = sum f_k(x_i) esas alınmış; Optuna ve Parçacık Sürü Optimizasyonu (PSO) ile hiperparametre araması yapılarak donanım kısıtlarına göre optimize edilmiştir.<br>\n• <strong>3. Tabular Transformer (FT-Transformer):</strong> Sayısal ve kategorik girdilerin Feature Tokenizer ile vektörleştirildiği, [CLS] token entegrasyonu ve Multi-Head Self-Attention (Çok Başlıklı Öz-Dikkat) katmanları ile değişkenler arası bağlamsal ilişkilerin öğrenildiği derin öğrenme mimarisidir.",
            "subsystems": [
                  {
  ...
  en: {
    ...
    projects: [
      ...
      },
      {
            ...
            "overview": "<strong>Project Credentials & TUSAŞ LIFT UP Program:</strong><br>\n• <strong>Project Title:</strong> Modeling and Comparing the Performance Dataset of a Combat Aircraft Using Machine Learning Methods<br>\n• <strong>Sponsor & Program:</strong> TUSAŞ (Turkish Aerospace Industries) 2025-2026 LIFT UP Industry-Oriented Graduation Project Program<br>\n• <strong>Industry Advisor:</strong> Zeynel Abidin AYDOĞAN (TUSAŞ - Aerospace Engineering)<br>\n• <strong>Academic Advisor:</strong> Dr. Aziz KABA (Eskişehir Technical University - Department of Pilotage)<br>\n• <strong>Project Team:</strong> Ahmet Soner GÜLEÇ (Lead Student Researcher - ESTÜ Avionics), Umut ÇÖRDÜK (ESTÜ Avionics), Ufuk ÖZKAN (ESTÜ Computer Engineering), Sema ÜNAL (ESTÜ Industrial Engineering).<br><br>\n<strong>Engineering Problem & Purpose:</strong><br>\nMission success in combat aircraft (e.g. MMU KAAN, F-18) relies on computing flight performance parameters (Specific Range, Fuel Flow, Rate of Climb, etc.) within millisecond precision across diverse altitude, speed, gross weight, and drag conditions. Traditional Lookup Table (LUT) methods are deterministic and explainable; however, memory requirements scale exponentially with multi-dimensional table resolution. Increasing table spacing introduces interpolation error risks.<br><br>\nThe core purpose of this project is to benchmark traditional Cubic Interpolation lookup methods against machine learning models (XGBoost and Tabular Transformer) in a hardware-aware manner regarding accuracy, inference latency, memory footprint, and CPU utilization over a master dataset extracted from Aircraft Flight Manual (AFM) nomograms via U-Net CNN segmentation and OCR.",
            "architecture": "<strong>Stage 1: Data Digitization Pipeline (U-Net & OCR):</strong><br>\n• <strong>U-Net Segmentation:</strong> Target performance curves from original AFM nomogram images were segmented at pixel precision from background grid lines using U-Net CNN architecture.<br>\n• <strong>OCR Integration:</strong> Numerical axis labels, tags, and coordinates were parsed via Optical Character Recognition and denormalized within a 2% error threshold.<br>\n• <strong>Master Dataset (Tidy Data):</strong> A structured table was built across 5 input variables (Mach Number, Altitude, Gross Weight, Drag Index, Engine Config) and target output (Specific Range).<br><br>\n<strong>Stage 2: Mathematical Modeling & AI Architectures:</strong><br>\n• <strong>1. Cubic Interpolation (Reference Baseline):</strong> Smooth 3rd-degree cubic splines maintaining derivative continuity.<br>\n• <strong>2. XGBoost Regression Model:</strong> Gradient boosted decision trees optimized via Optuna & Particle Swarm Optimization (PSO) under hardware constraints.<br>\n• <strong>3. Tabular Transformer (FT-Transformer):</strong> Deep learning architecture projecting continuous/categorical inputs via Feature Tokenizer and [CLS] token processed by Multi-Head Self-Attention layers.",
            "subsystems": [
                  {
                        "title": "U-Net & OCR Digitization Pipeline",
